...
namespace AcfDuplicateRepeater\AutoUpdate;
...
abstract class AutoUpdate extends Core\Singleton {
	...
	public function source_selection( $source, $remote_source, $wp_upgrader, $hook_extra ) {
		if ( isset( $hook_extra['plugin'] ) && $hook_extra['plugin'] === plugin_basename( ACF_DUPLICATE_REPEATER_FILE ) ) {
			// $source: filepath
	...
	public function pre_set_transient( $transient ) {
		...
		if ( $release_info = $this->get_release_info() ) {
			$plugin 		= plugin_basename( ACF_DUPLICATE_REPEATER_FILE );
			$slug			= basename( ACF_DUPLICATE_REPEATER_DIRECTORY );
			$plugin_info	= get_plugin_data( ACF_DUPLICATE_REPEATER_FILE );
